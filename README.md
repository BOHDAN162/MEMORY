...
- Copy `.env.example` to `.env.local` and fill in your `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` from the Supabase dashboard.
- Auth helpers live in `lib/supabase/` (client, server, and middleware clients). Middleware keeps Supabase session cookies in sync for SSR.
